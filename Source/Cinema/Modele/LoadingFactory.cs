...
{
    /// <summary>
    /// Instancie différents objet lors du chargement de donnée depuis un fichier
    /// </summary>
    public class LoadingFactory : IFactory
    {
        /// <summary>
            ...
            IEnumerable<Streaming> listeStream, IEnumerable<KeyValuePair<User, Avis>> listeAvis = null)
        {
            ...
            if (listeAvis is null) return le;

            foreach (var (user, avis) in listeAvis)
                le.AjouterAvis(user, avis);
        ...
        {
            var oeuvres = listeOeuvres.ToList();
            ...
            object comp = type switch
            {
                TRILOGIE when listePersonnes == null => new Trilogie(titre, dateDeSortie, lienImage, synopsis, theme, familleF,
                    new Dictionary<string, IEnumerable<KeyValuePair<Personne, string>>>(), oeuvres),

                TRILOGIE => new Trilogie(titre, dateDeSortie, lienImage, synopsis, theme, familleF, listePersonnes, oeuvres),
                ...
                UNIVERS => new Univers(titre, dateDeSortie, lienImage, synopsis, theme, familleF, listePersonnes, oeuvres),

                _ => null
            };

            if (comp is not Composite cmp) return null;
            if (cmp is Serie) cmp.AjouterOeuvres(oeuvres);
            else if (cmp.ReOeuvres.Count == 0) return null;

            foreach (var pers in cmp.Personnes.Select(p => p.Value).SelectMany(p => p.Keys)) 
                pers.AjouterOeuvre(cmp);

            if (listeAvis is null) return cmp;

            foreach (var (user, avis) in listeAvis)
                cmp.AjouterAvis(user, avis);
        ...
        /// <seealso cref="Personne"/>
        public Personne CreerPersonne(string prenom, string nom, string bio, string nat, string lien, DateTime date) => new(nom, prenom, bio, nat, lien, date);
        ...
        /// <seealso cref="User"/>
        /// <seealso cref="Plateformes"/>
        public User CreerUser(string pseudo, string mdp, string photo, bool famille = false, bool admin = false, IEnumerable<Plateformes> plateformes = null)
        {
            var user = new User(pseudo, mdp) {IsAdmin = admin};
            user.ChangerProfil(pseudo, photo, famille, plateformes);

            return user;
        }

        /// <summary>
        /// Permet d'ajouter les listes d'envies et de récemment consultés à un User 
        /// </summary>
        /// <param name="user">Le User</param>
        /// <param name="envie">La liste d'envie</param>
        /// <param name="consulte">La liste de récemment consulté</param>
        /// <seealso cref="User"/>
        /// <seealso cref="Oeuvre"/>
        /// <seealso cref="Personne"/>
        public void ModifierLiseUser(User user, IEnumerable<Oeuvre> envie = null, IEnumerable<object> consulte = null)
        {
            foreach (var oeu in envie!) user.AjouterEnvie(oeu);
            foreach (var obj in consulte!) user.AjouterConsulte(obj);
        }
    }
}